/**
 * React hook for standalone P2P file transfers
 * 
 * This hook provides a complete interface for P2P file transfers without requiring a server,
 * using the StandaloneP2PService for direct transfers.
 */

import { useState, useEffect, useCallback, useRef } from 'react';
import { StandaloneP2PService } from '../services/standalone-p2p.service';
...
/**
 * Hook for standalone P2P file transfers
 */
export function useStandaloneP2PTransfer(options: UseP2PTransferOptions = {}): UseP2PTransferReturn {
  ...
  const p2pServiceRef = useRef<StandaloneP2PService | null>(null);
  const currentFileRef = useRef<File | null>(null);

  // Initialize P2P service
  useEffect(() => {
    const finalConfig = { ...DEFAULT_CONFIG, ...config };
    p2pServiceRef.current = new StandaloneP2PService(finalConfig);
  ...
  const isSupported = StandaloneP2PService.isSupported();
...
export default useStandaloneP2PTransfer;